...
# 시작 날짜가 끝 날짜보다 이전인 경우, 다시 선택하도록 다이얼로그 표시
if start_datetime > end_datetime:
    st.error("시작 날짜는 끝 날짜보다 이전이어야 합니다. 다시 선택해주세요.")
    st.stop()
...
# y 축 리미트 설정
fig.update_layout(yaxis=dict(range=[avg_value - 7, avg_value + 7]))
